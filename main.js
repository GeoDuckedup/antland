...
function addFood(x, z, kind = 'crumb', amount = 52, size = 1.5) {
  ...
  const food = { x, z, kind, amount, initial: amount, size, mesh };
  foods.push(food);
...
function spawnAnt(initialPopulation = false) {
  ...
  const ant = {
    ...
    location: 'surface',
    tendency: random() < 0.54 ? 'forager' : random() < 0.68 ? 'nurse' : 'excavator',
  ...
  ants.push(ant);
  if (random() < 0.28) beginNestJourney(ant, ant.tendency === 'nurse' ? 'nursery' : 'rest', true);
...
let laborClock = 99;
const colonyLabor = { targetExcavators: 0, assignedExcavators: 0 };
function updateLaborAssignments(dt) {
  laborClock += dt;
  if (laborClock < 2.5) return;
  laborClock = 0;
  const projects = activeConstructionProjects();
  const target = projects.length > 0 ? clamp(Math.round(ants.length * 0.065), 8, 14) : 0;
  colonyLabor.targetExcavators = target;

  const committed = ants.filter((ant) => ant.assignedRole === 'excavator' && (ant.soilCargo || ant.nestRouteKey === 'excavation'));
  for (const ant of ants) {
    if (ant.soilCargo || ant.nestRouteKey === 'excavation') continue;
    ant.assignedRole = ant.tendency === 'nurse' ? 'nurse' : 'forager';
  }
  const candidates = ants
    .filter((ant) => !ant.carrying && !ant.soilCargo && ant.nestRouteKey !== 'excavation')
    .sort((a, b) => {
  ...
  const additionalNeeded = Math.max(0, target - committed.length);
  for (let i = 0; i < Math.min(additionalNeeded, candidates.length); i++) candidates[i].assignedRole = 'excavator';
...
function moveSurfaceDirected(ant, dt, speedFactor = 1) {
  ...
  const velocity = ant.speed * speedFactor * (1 - weather.rain * 0.18);
...
function updateAnt(ant, dt) {
  ...
  if (ant.carrying) {
    ...
    if (food) {
      ...
      if (strongest > 0.035 && random() < 0.87) {
  ...
  const velocity = ant.speed * rainSlow * pauseFactor * (ant.carrying ? 1.08 : 1);
  ant.x += Math.cos(ant.heading) * velocity * dt;
  ant.z += Math.sin(ant.heading) * velocity * dt;
  ant.distanceTraveled += velocity * dt;
...
function updateAntNote() {
  ...
  antNoteFacts.innerHTML = `
    ...
    <dt>cargo</dt><dd>${selectedAnt.soilCargo ? 'excavated soil' : selectedAnt.carrying ? 'food fragment' : 'none'}</dd>
...
function updateWeather(dt) {
  if (simTime > weather.nextRain && weather.rainTimer <= 0) startRain(rand(9, 15));
...
function update(dt) {
  ...
  spawnClock += dt;
  foodClock += dt;
  // Population remains stable until the queen/brood phase supplies real new workers.
  if (foodClock > 27 && foods.filter((f) => f.amount > 0).length < 4) {
    foodClock = 0;
    addFood(rand(-12, 13), rand(-9, 9), random() < 0.32 ? 'berry' : 'crumb', Math.floor(rand(42, 78)), rand(1.25, 1.7));
  }
  updateWeather(dt);
  updatePheromones(dt);
...
window.addEventListener('keydown', async (event) => {
  ...
  if (event.key.toLowerCase() === 'b' && !event.repeat) {
    ...
    if (goingBelow) {
      cameraRig.yaw = 1.22;
      cameraRig.target.x = NEST.x + 0.8;
      cameraRig.target.z = NEST.y + 0.25;
    }
  }
...
window.render_game_to_text = () => {
  ...
  return JSON.stringify({
    ...
    colony: { ants: ants.length, surfaceVisible: ants.length - insideNest, insideNest, carrying, foodDelivered: delivered, states },
    nest: { x: NEST.x, z: NEST.y },
    foodSources: foods.filter((food) => food.amount > 0).map((food) => ({
      kind: food.kind, x: Number(food.x.toFixed(1)), z: Number(food.z.toFixed(1)), amount: food.amount,
    })),
    ...
    selectedAnt: selectedAnt ? {
      ...
      cargo: selectedAnt.soilCargo ? 'soil' : selectedAnt.carrying ? 'food' : 'none',
    ...
    controls: 'click an ant to select/follow; Esc releases; WASD/arrow keys move; Q/E descend/ascend; drag orbit; B toggles above/below; [ and ] change time speed; 0 resets time; right-drag pan; wheel zoom; empty click adds food; shift-click stone; R rain; space pause; F fullscreen',
  });
};
